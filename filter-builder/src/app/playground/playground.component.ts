...
@Component({
    ...
    providers: [FieldValueEmitter, NgDestroyer],
})
export class PlaygroundComponent {
    ...
    constructor(
        ...
        const categoryPair = new FilterPair(
            new SelectField(new FieldId(2), 'Category', of(['a', 'b', 'c'])),
            new StringValue('a'),
        );

        const depColors$ = combineLatest([colors$, categoryPair.currentValue.value$]).pipe(
            map(([colors, categoryValue]) => {
                const term = categoryValue.value?.toLowerCase() ?? null;
                return (!!term ? colors.filter((color) => color.category === term) : colors).map(
                    (color) => color.color,
                );
            }),
        );
        ...
        this.filter = new Filter(new FilterId(1), [
            new FilterPair(new TextualField(new FieldId(0), 'Name'), new StringValue('text')),
            new FilterPair(new NumericField(new FieldId(1), 'Age'), new NumberValue(1)),
            categoryPair,
            new FilterPair(
                new SelectField(new FieldId(3), 'Color', depColors$),
                new StringValue(null),
            ),
            new FilterPair(new CheckboxField(new FieldId(4), 'Archived'), new BooleanValue(null)),
            new FilterPair(
